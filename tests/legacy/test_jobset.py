...
from scrapinghub.legacy import JobSet


def test_jobset_init(jobset: JobSet) -> None:
    assert isinstance(jobset.project, Project)
    assert jobset.params == {'param': 'value'}
    assert jobset._jobs is None


def test_jobset_repr(jobset: JobSet) -> None:
    s = repr(jobset)
    assert s == "JobSet(Project(Connection('testkey'), 12345), param=value)"


def test_jobset_iter(jobset: JobSet) -> None:
    jobs_data = [{'id': '1/2/3', 'job_data': 'info'}]
    jobset._jobs = jobs_data
    ...
    assert jobs[0].info == jobs_data[0]


def test_jobset_count(jobset: JobSet) -> None:
    jobset._get = mock.Mock()  # type: ignore[method-assign]
    jobset._get.return_value = {'total': 100}
    assert jobset.count() == 100
    assert jobset._get.call_args_list == [(('jobs_count', 'json'), {})]


def test_jobset_update(jobset: JobSet) -> None:
    jobset._post = mock.Mock()  # type: ignore[method-assign]
...
def test_jobset_stop(job_mock: mock.MagicMock, jobset: JobSet) -> None:
...
@mock.patch('scrapinghub.legacy.Job')
def test_jobset_delete(job_mock: mock.MagicMock, jobset: JobSet) -> None:
    jobset._jobs = [{'id': '1/2/3'}]
    jobset.delete()
    assert job_mock.return_value.delete.called


def test_jobset_load_jobs_already_loaded(jobset: JobSet) -> None:
    jobset._jobs = [{'id': '1/2/3'}]
    jobset._load_jobs()


def test_jobset_load_jobs_stop_iteration(jobset: JobSet) -> None:
    jobset._get = mock.Mock()  # type: ignore[method-assign]
...
def test_jobset_load_jobs_bad_status(jobset: JobSet) -> None:
...
def test_jobset_load_jobs_base(jobset: JobSet) -> None:
...
def test_jobset_request_proxy(jobset: JobSet) -> None:
    assert jobset._request_proxy == jobset.project


def test_jobset_add_params_void(jobset: JobSet) -> None:
    assert jobset._add_params({}) == {'param': 'value'}


def test_jobset_add_params_base(jobset: JobSet) -> None:
